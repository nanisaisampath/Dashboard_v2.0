...
export const DashboardProvider = ({ children }: { children: ReactNode }) => {
  ...
  const selectTicketsByCategory = (category: string, value: string) => {
    ...
    console.log(`Selecting tickets with ${category}: ${value}`);
    ...
    if (dataField === 'technology') {
      ...
      const tickets = filteredData.filter(ticket => {
        ...
        console.log(`Comparing ticket type ${ticketTypeValue} with ${value}`);
        return ticketTypeValue === value;
      });
      console.log(`Found ${tickets.length} tickets with type ${value}`);
      setSelectedTickets(tickets);
    } else {
      const tickets = filteredData.filter(ticket => {
